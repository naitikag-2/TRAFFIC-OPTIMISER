...
// ═══════════════════════════════════════════
// 10. SCROLL-DRIVEN ARCHITECTURE STORY
// ═══════════════════════════════════════════
function initArchStory() {
  const wrapper = document.getElementById('archScrollWrapper');
  if (!wrapper) return;

  const STAGES = [
    { title: "Capture", desc: "Live camera feeds provide the raw traffic data." },
    { title: "Understand", desc: "Edge AI detects and classifies vehicles in real time." },
    { title: "Measure", desc: "Vehicle density is converted into actionable traffic states." },
    { title: "Optimize", desc: "Reinforcement learning continuously adjusts signal timing." },
    { title: "Command", desc: "Optimized timing decisions are transmitted to the controller." },
    { title: "Execute", desc: "Physical traffic signals respond to the AI-generated command." }
  ];
  ...
  const vizs = document.querySelectorAll('.stage-viz');
  const titleEl = document.getElementById('archStoryTitle');
  const descEl = document.getElementById('archStoryDesc');
  ...
  function setStage(idx) {
    ...
    setTimeout(() => {
      ...
      descEl.textContent = STAGES[idx].desc;
      titleEl.style.opacity = '1';
      descEl.style.opacity = '1';
    }, 200);

    // Update Dots, Nodes, Connectors, Viz
    for (let i = 0; i < 6; i++) {
      dots[i].classList.toggle('active', i === idx);
      
      // Node is active if it's the current stage, but let's highlight nodes leading up to it
      if (i <= idx) nodes[i].classList.add('active');
      else nodes[i].classList.remove('active');
      
      if (i < 5) {
        if (i < idx) connectors[i].classList.add('active');
        else connectors[i].classList.remove('active');
      }

      vizs[i].classList.toggle('active', i === idx);
    }
    
    // Trigger specific animations based on stage
    triggerStageAnimations(idx);
  }

  function triggerStageAnimations(idx) {
    if (idx === 1) { // Edge AI
      animateValue(document.getElementById('sv-cars'), 0, 42, 1000);
      animateValue(document.getElementById('sv-buses'), 0, 6, 1000);
      animateValue(document.getElementById('sv-trucks'), 0, 9, 1000);
      animateValue(document.getElementById('sv-total'), 0, 57, 1000);
    } else if (idx === 2) { // Matrix
      generateMatrixDemo();
      animateFloat(document.getElementById('sv-dn'), 0, 0.72, 1000);
      animateFloat(document.getElementById('sv-ds'), 0, 0.41, 1000);
      animateFloat(document.getElementById('sv-de'), 0, 0.83, 1000);
      animateFloat(document.getElementById('sv-dw'), 0, 0.56, 1000);
    } else if (idx === 3) { // Optimizer
      // Simulate RL finding better values
      setTimeout(() => {
        document.getElementById('sv-tn').textContent = '36s';
        document.getElementById('sv-ts').textContent = '14s';
        document.getElementById('sv-te').textContent = '29s';
        document.getElementById('sv-opt-status').textContent = 'OPTIMIZATION COMPLETE';
        document.getElementById('sv-opt-status').style.color = '#10B981';
        document.getElementById('sv-opt-status').style.background = '#D1FAE5';
      }, 800);
    } else if (idx === 5) { // Hardware
      setTimeout(() => {
        document.getElementById('sv-tl-r').classList.remove('active');
        document.getElementById('sv-tl-y').classList.add('active');
        setTimeout(() => {
          document.getElementById('sv-tl-y').classList.remove('active');
          document.getElementById('sv-tl-g').classList.add('active');
        }, 600);
      }, 500);
    }
  }
  ...
  function generateMatrixDemo() {
    const grid = document.getElementById('sv-grid');
    if(!grid) return;
    grid.innerHTML = '';
    const states = ['dc-low', 'dc-med', 'dc-high'];
    for(let i=0; i<16; i++) {
      const cell = document.createElement('div');
      cell.className = 'density-cell ' + states[Math.floor(Math.random()*states.length)];
      grid.appendChild(cell);
      
      // Animate some randomly
      setTimeout(() => {
        cell.className = 'density-cell ' + states[Math.floor(Math.random()*states.length)];
  ...
  window.addEventListener('scroll', () => {
    // Check if the arch tab is visible
    ...
    let stage = Math.floor(progress * 6);
    if (stage >= 6) stage = 5;
